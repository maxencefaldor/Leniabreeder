...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    def add(
        ...
        batch_of_observations: jnp.array,
        ...
        new_repertoire_genotypes = jax.tree_util.tree_map(
            lambda x, y: x.at[batch_of_indices.squeeze(axis=-1)].set(y),
            self.genotypes,
            batch_of_genotypes,
        )

        # compute new fitness, descriptors and observations
        ...
        new_observations = jax.tree_util.tree_map(
            lambda x, y: x.at[batch_of_indices.squeeze(axis=-1)].set(y),
            self.observations,
    ...
    def init(
        ...
        first_observation = jax.tree_util.tree_map(lambda x: x[0], observations)

        # create a repertoire with default values
        repertoire = cls.init_default(genotype=first_genotype, centroids=centroids, observation=first_observation)

        # add initial population to the repertoire
        new_repertoire, _ = repertoire.add(genotypes, descriptors, fitnesses, observations, extra_scores)
    ...
    def init_default(
        ...
        Args:
            genotypes: the typical genotypes that will be stored.
        ...
        # default observations is all nan
        default_observations = jax.tree_util.tree_map(
            lambda x: jnp.nan * jnp.zeros(shape=(num_centroids,) + x.shape, dtype=x.dtype),
            observation,
        )
